...
def resampling_visualizations(self):
    '''
    ...
    data = pd.DataFrame(columns=["Sensor_Value"],data=self.time_series["Sensor_Value"])
    ...
    '''
    transform = np.sqrt(data) # throws a warning due to negative values
    plt.figure(1)
    # line plot
    plt.subplot(211)
    plt.plot(transform)
    ...
    copied_data['log_Sensor_Value'] = \
        np.log(copied_data['Sensor_Value']) # throws a warning for 0 values
    plt.figure(1)
    # line plot
    plt.subplot(211)
